...
/// thread.
void Interrupt::Enable() { SetLevel(INT_ON); }
...
void Interrupt::Schedule(VoidFunctionPtr handler, void *arg, unsigned long fromNow, IntType type) {
    ...
    if (UINT_MAX - stats->totalTicks < fromNow) {
        DEBUG('x',
              "WARNING: total tick count is too large"
              " and will be reset.\n");
        RestartTicks();
    }
#else
...
IntStatus Interrupt::GetLevel() const { return level; }

MachineStatus Interrupt::GetStatus() const { return status; }

void Interrupt::SetStatus(MachineStatus st) { status = st; }
